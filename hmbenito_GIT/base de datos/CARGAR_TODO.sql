CREATE TABLE libros (
    id_libro INTEGER PRIMARY KEY AUTOINCREMENT, -- O INT AUTO_INCREMENT para MySQL
    titulo VARCHAR(255) NOT NULL,
    id_autor INTEGER,
    año_publicacion INTEGER,
    genero VARCHAR(50),
    FOREIGN KEY (id_autor) REFERENCES autores(id_autor) -- Esto crea la relación
);

CREATE TABLE autores (
    id_autor INTEGER PRIMARY KEY AUTOINCREMENT, -- O INT AUTO_INCREMENT para MySQL
    nombre VARCHAR(100) NOT NULL,
    nacionalidad VARCHAR(50)
);

CREATE TABLE usuarios (
    id_usuario INTEGER PRIMARY KEY AUTOINCREMENT, -- O INT AUTO_INCREMENT para MySQL
    nombre VARCHAR(100) NOT NULL,
    email VARCHAR(100) UNIQUE, -- UNIQUE asegura que no haya emails duplicados
    fecha_registro DATE
);
